...
def vitals_df(data):
    ...
    vitals_todo = ['PULSE OXIMETRY', 'TEMPERATURE', 'PULSE', 'RESPIRATIONS', 'URINE OUTPUT', 'BLOOD PRESSURE', 'R ARTERIAL LINE BLOOD PRESSURE']
    ...
    for vital_tmp in vitals_todo:
        ...
        elif vital_tmp == 'R ARTERIAL LINE BLOOD PRESSURE':
            vital_data['bp_art_systolic'] = vital_data.MeasureTXT.apply(lambda x: x.split('/')[0])
            vital_data['bp_art_diastolic'] = vital_data.MeasureTXT.apply(lambda x: x.split('/')[1])
            vital_data = vital_data[['bp_art_systolic', 'bp_art_diastolic']]
        else:
...
def labs_df(data):
    ...
    labs_todo = ['PO2, ARTERIAL', 'PO2, VENOUS', 'PCO2, ARTERIAL', 'PCO2, VENOUS', 'HCO3, ARTERIAL', 'HCO3, VENOUS', 'PH, ARTERIAL', 'PH, VENOUS']
